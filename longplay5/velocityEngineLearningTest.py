...
playerAccelerations = ["./playerAccelerationTrack.csv"]
spriteAccelerations = ["./spriteAccelerationTrack.csv"]
spritesListings = ["./frame4Descriptions1-1.csv"]
cameraPositions = ["./camera4Positions1-1.csv"]
startAndEndFrames =[[1769, 1788]]#220 (Video 4) 338 is precoin make
#Video 5: 395-1939 
#4: 220-1789
...
for indexOfThings in range(len(playerAccelerations)):
	...
	#currEngine.currLevel = level1Map
					...
					for rule in engineCheckNode.engine.totalRules:
						...
						if len(possibleConditionsSet.intersection(goalConditionSet))!=0:
							...
							if usefulModification:
								...
								newRule = rule.clone()
								newRule.preconditions.append(RulePrecondition(list(prevFrameSet.intersection(currConditionSet)), newRule.change[0].frameObject))
								modifyEngine.ReplaceRule(rule, newRule, engineCheckNode.engine)
								newEngineList.append(modifyEngine)
