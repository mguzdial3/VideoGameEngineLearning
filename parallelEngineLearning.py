...
cameraPositions =  "./cameraPositions5.csv"
startAndEndFrames =[395,495]
screenSize = [412,360]
theta = 0.05 #How close must each prediction be
...
minFrameIndex = startAndEndFrames[indexOfThings][0]
maxFrame = startAndEndFrames[indexOfThings][1]
...
allFramesInfo[indexOfThings] = changeFrames
...
for indexOfThings in range(len(playerAccelerations)):
	currIndexes.append(startAndEndFrames[indexOfThings][0]+1)
...
for indexOfThings in range(len(playerAccelerations)):
	if currIndexes[indexOfThings]==startAndEndFrames[indexOfThings][1]:
		notDone = False
		break
...
for indexOfThings in range(len(playerAccelerations)):
	currIndex = currIndexes[indexOfThings]
	print "CURR INDEX: "+str(currIndex)

	thisFrameScanned = currIndex-startAndEndFrames[indexOfThings][0]
	if thisFrameScanned>totalFramesScanned:
		totalFramesScanned = thisFrameScanned

	for rule in currEngine.totalRules:
		print str(rule)

	prevFrame = allFramesInfo[indexOfThings][currIndexes[indexOfThings]-1]
	currFrame = allFramesInfo[indexOfThings][currIndexes[indexOfThings]]

	newFrames = Predict(allFramesInfo[indexOfThings][currIndexes[indexOfThings]-1], currEngine, allFramesInfo[indexOfThings][currIndexes[indexOfThings]])
	foundFrame = False
	...
	for frame in newFrames:
		frame.frameScreen = {}
		thisDist = FrameDistance(frame, allFramesInfo[indexOfThings][currIndex])
		print "    Check Dist: "+str(thisDist)+". Player: "+str(frame.player)+" vs "+str(currFrame.player)
		print "This Dist: "+str(thisDist)
		if thisDist<=minVal:#Less than theta
			#Update currIndexes
			currIndexes[indexOfThings]+=1
			foundFrame = True

		elif thisDist<minFrameDist:
			minFrame = frame
			minFrameDist = thisDist

	print "Starting engine search between: "+str([currIndex-1, currIndex]) +" cause of "+str(thisDist)
	...
	if not foundFrame:
		...
		while notLearned:
			timesAttempted+=1
			engineCheck = engineQueue.get()
			print "Curr Engine Cost: "+str(engineCheck[0])
			engineCheckNode = engineCheck[1]
			visitedEngineList.append(engineCheckNode.engine)
			currEngine = engineCheckNode.engine

			#Check and see if we made it
			prevFrame = allFramesInfo[indexOfThings][currIndex-1]
			currFrame = allFramesInfo[indexOfThings][currIndex]

			if not firstRun:
				newFrames = Predict(prevFrame, engineCheckNode.engine, currFrame)
				currConditionSet = []
				minFrame = None
				minFrameDist = float("inf")
				for frame in newFrames:
					#Check and see if we're close enough
					frameDist = FrameDistance(frame, currFrame)
			...
			if timesAttempted>timesAttemptedMax:
				currEngine = bestEngineThisAttempt
				notLearned = False
			...
			if notLearned:
				...
				postVelocityFrames = PhysicsPredict(prevFrame, engineCheckNode.engine, currFrame)
				...
				goalConditionSet = set(currFrame.ReallyGetConditions())#set(changeFrames[currIndex].ReallyGetConditions()).difference(currConditionSet)
				#Transition Function: Generate new engines from this engine
				#How to pick the rules to add?
				#Modifiction Rules

				for condition in currConditionSet:
					if condition not in goalConditionSet:
						print "CURR CONDITION: "+str(condition)
				#print ""
				for goalCondition in goalConditionSet:
					if goalCondition not in currConditionSet:
				...
				for rule in engineCheckNode.engine.totalRules:
					...
					if len(possibleConditionsSet.intersection(goalConditionSet))!=0:
						...
						if usefulModification:
							if modifyEngine not in newEngineList:
					...
					for spriteName in prevFrame.spritesContained:
						...
						if not isinstance(potentialRule.change[0], VariableCondition):
							...
							if len(possibleConditionsSet.intersection(goalConditionSet))!=0:
								modifyEngine = Engine(currEngine)
								if potentialRule not in modifyEngine.rules:
									modifyEngine.AddRule(potentialRule, prevFrame, currFrame)
									if modifyEngine not in newEngineList:
										newEngineList.append(modifyEngine)
									else:
										print "Rejected modify engine 7"
				...
				for goalCondition in goalConditionSet:
					if isinstance(goalCondition, AnimationCondition):
						...
						if goalCondition.frameObject not in minFrame.frameObjects and ("megaman" not in goalCondition.spriteName or ("megaman" in goalCondition.spriteName and "megaman" not in minFrame.spritesContained)) :
							...
							if not newRule in modifyEngine.rules:
								modifyEngine.AddRule(newRule, prevFrame, currFrame)
								if modifyEngine not in newEngineList:
									newEngineList.append(modifyEngine)
									handledConditions.append(goalCondition)
								else:
									print "Rejected modify engine 6"
					...
					if isinstance(condition, VelocityXCondition):
						for goalCondition in goalConditionSet:
							if isinstance(goalCondition, VelocityXCondition):
								if goalCondition.spriteName==condition.spriteName and goalCondition.velocityValue!=condition.velocityValue:
									...
									if newRule not in modifyEngine.rules:
										#if goalCondition.spriteName=="largeHill":
										...
										if modifyEngine not in newEngineList:
											...
											print "Rejected modify engine 5"
								...
								if goalCondition.spriteName==condition.spriteName and goalCondition.velocityValue!=condition.velocityValue:
									...
									if newRule not in modifyEngine.rules:
										...
										if modifyEngine not in newEngineList:
											...
											print "Rejected modify engine 4"

				
				for condition in currConditionSet:
					#Check if something should disappear
					if isinstance(condition, AnimationCondition):
						if condition.frameObject not in currFrame.frameObjects and ("megaman" not in condition.spriteName or ("megaman" in condition.spriteName and currFrame.player==None)):
							...
							if not newRule in modifyEngine.rules:
								...
								if modifyEngine not in newEngineList:
									...
									print "Rejected modify engine 3"

					#Check all other rules to add
					for goalCondition in goalConditionSet:
						if type(condition) is type(goalCondition):
							if condition not in handledConditions and goalCondition not in handledConditions:
								if isinstance(condition, VariableCondition):
									if condition.variableValue!=goalCondition.variableValue and condition.variableName==goalCondition.variableName:
										...
										if newRule not in modifyEngine.rules:
											...
											if modifyEngine not in newEngineList:
												...
												print "Rejected modify engine 2"
									...
									if (condition.spriteName!=goalCondition.spriteName and abs(condition.x-goalCondition.x)==0 and abs(condition.y-goalCondition.y)==0) or (condition.spriteName==goalCondition.spriteName and (condition.width!=goalCondition.width or condition.height!=goalCondition.height)):
										...
										if newRule not in modifyEngine.rules:
											...
											if modifyEngine not in newEngineList:
												...
												print "Rejected modify engine 1"
								

				#Add new engine nodes
				print "New Engine List Length: "+str(len(newEngineList))+" vs Engine Queue: "+str(len(engineQueueList))+" vs Visited: "+str(len(visitedEngineList))
				smallestMin = float("inf")
				for potentialEngine in newEngineList:
					if (potentialEngine not in engineQueueList) and (potentialEngine not in visitedEngineList):							
						newFrames = Predict(prevFrame, potentialEngine, currFrame)
						#prevToPredictedDiff = thisConditionSet.difference(prevFrameSet)
						minDist = float("inf")
						closestFrame = None

						for frame in newFrames:
							thisDist = FrameDistance(frame, currFrame)

							if thisDist<minDist:
								minDist = thisDist
								thisConditionSet = set(frame.GetFrameConditions())
								closestFrame = frame
						if minDist<smallestMin:
							smallestMin = minDist
				...
				print "Smallest min: "+str(smallestMin)
		if not notLearned:
			if timesAttempted>timesAttemptedMax:
				currIndexes[indexOfThings] +=1
				print "Moving on for now"
				break
			else:
				currIndexes[indexOfThings] = startAndEndFrames[indexOfThings][0]+1
				print "Reset to Starts"
				break

pickle.dump(currEngine, open("./Engines/finalEngine"+str(maxFrame-1)+engineDesignation+".p", "wb"))
